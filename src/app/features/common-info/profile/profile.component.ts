...
export class ProfileComponent implements OnInit {
  ...
  updateInfo(form){
    ...
    this.loading = true;
    this.userService.update(this.uid, form.value).subscribe(
      data => {
        this.toastr.success('Updated your profile successfully! Please login again', 'Success');
  ...
  updatePassword(form){
    this.isPwdSubmitted = true;
    if (!form.valid) {
      return;
    }
    this.loadingPwd = true;
    this.userService.changePassword(this.uid, form.value).subscribe(
      data => {
        this.toastr.success('Updated your password successfully! Please login again', 'Success');
        this.authService.logout();
      },
      error => {
        this.toastr.error(this.commonService.convertReqErr2String(error.error), 'Error');
